...
def compute_decision_overhead_metrics(logs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Counts R (reasoning), W (waiting), and I (interrupted) states as decision overhead.
    X (executing) is not counted.
    Also computes wait time and decision time per agent and per step.
    """
    ...
    # States that count as decision overhead (R, W, I - not X)
    DECISION_STATES = {'reasoning', 'waiting', 'interrupted'}
    
    # Get all agents
    agents = list(agent_states.keys())
    n_agents = len(agents)
    
    # Compute per-agent decision overhead D_i (count of R and I states only)
    D_i = {}
...
def compute_planning_metrics(logs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Plan Interruption Count P_int
    # Interruption = plan ended with status != 'success' or was abandoned
    P_int = 0
    for plan in plan_history:
    ...
    for agent_id, plans in plans_per_agent.items():
        ...
        sorted_plans = sorted(plans, key=lambda p: p.get('start_step', 0))
    ...
    max_step = 0
    for plan in plan_history:
        max_step = max(max_step, plan.get('end_step', 0))
    ...
    for step in range(max_step + 1):
        ...
        for plan in plan_history:
            ...
            start = plan.get('start_step', 0)
            end = plan.get('end_step', 0)
            if start <= step <= end:
...
def compute_constraint_metrics(logs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Uses task_states log which tracks spatial, temporal, dependency, participation.
    ...
    participation_improved_tasks = set()
    participation_worsened_tasks = set()
    ...
    for step_data in task_states:
        ...
        participation_improved_tasks.update(constraint_changes.get('participation', {}).get('improved_tasks', []))
        participation_worsened_tasks.update(constraint_changes.get('participation', {}).get('worsened_tasks', []))
    ...
    total_participation_improved = len(participation_improved_tasks)
    total_participation_worsened = len(participation_worsened_tasks)
    
    # Aggregate: unique tasks with any improvement/worsening
    tasks_with_any_improvement = spatial_improved_tasks | temporal_improved_tasks | dependency_improved_tasks | participation_improved_tasks
    tasks_with_any_worsening = spatial_worsened_tasks | temporal_worsened_tasks | dependency_worsened_tasks | participation_worsened_tasks
    ...
    return {
        ...
        'participation': {'improved': total_participation_improved, 'worsened': total_participation_worsened},
...
def print_metrics_summary(metrics: Dict[str, Any]):
    ...
    print(f"  Participation: +{c.get('participation', {}).get('improved', 0)} / -{c.get('participation', {}).get('worsened', 0)}")
...
def save_metrics_csv(metrics: Dict[str, Any], output_path: str):
    ...
    participation_imp = c.get('participation', {}).get('improved', 0)
    participation_wor = c.get('participation', {}).get('worsened', 0)
    ...
    rows.append(['C_participation_ratio', 'Participation constraint improvement ratio', improvement_ratio(participation_imp, participation_wor)])
