...
- plain-text message generation for topology protocols
...
class BaseLLMAgent(Agent):
    ...
    def _plan_context_prefix(self) -> str:
        """Topology context placed before the observation, e.g. the leader's directive."""
        return ""
    ...
    def _generate_plan_from_messages(
        ...
        """Call the LLM and parse a symbolic plan; fall back to a fixed plan on failure."""
    ...
    def _generate_fallback_plan(self) -> SymbolicPlan:
        """Simple fallback plan when the LLM call fails (CUBE version)."""
        return SymbolicPlan(
    ...
    def decide_interrupt(
        ...
        call this after their own protocol steps, e.g. acknowledging the leader.
