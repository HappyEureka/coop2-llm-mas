...
class SymbolicPlan:
    ...
    def complete_action(self, action_index: int, step: int, success: bool, failure_reason: Optional[str] = None) -> bool:
        """
...
class SymbolicPlanLogger:
    ...
    def log_action_completed(self, plan: SymbolicPlan, action: SymbolicAction, step: int, 
        ...
        plan.complete_action(action_index, step, success, failure_reason)
    ...
    def get_statistics(self) -> Dict:
        ...
        total_duration = sum(p["duration"] for p in self.plan_history if p["duration"])
    ...
    def step(self, observation: Any, env_step: int, action_results: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        if plan.status == SymbolicPlanStatus.PENDING:
            ...
            if old_plan is not None and old_plan is not plan:
                if old_plan.status == SymbolicPlanStatus.EXECUTING:
