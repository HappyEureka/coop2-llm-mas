"""Plot CUBE score versus decision and communication cost."""
...
def plot_tradeoff(input_csv: Path, output: Path) -> Path:
    ...
    for col_idx, (x_key, x_ci_key, label) in enumerate(column_specs):
        ...
        ax.set_xlim(left=-0.02 * max(x_limits[x_key], 1.0), right=max(x_limits[x_key], 1.0))
        ax.set_title("Decision cost" if col_idx == 0 else "Communication cost", fontweight="bold", pad=5)
        ax.text(0.02, 0.94, chr(ord("A") + col_idx), transform=ax.transAxes, ha="left", va="top", fontsize=10.5, fontweight="bold", color="#111827")

    model_handles = [
        mlines.Line2D([], [], marker="o", linestyle="None", markerfacecolor=color, markeredgecolor="#111111", markersize=7.2, label=MODEL_LABELS.get(model, model))
        for model, color in MODEL_COLORS.items()
        if any(row["model"] == model for row in rows)
    ]
    topology_handles = [
        mlines.Line2D([], [], marker=marker, linestyle="None", markerfacecolor="#E5E7EB", markeredgecolor="#111111", markersize=7.2, label=TOPOLOGY_LABELS.get(topology, topology.title()))
        for topology, marker in TOPOLOGY_MARKERS.items()
        if any(row["topology"] == topology for row in rows)
    ]
    agent_handles = [
        mlines.Line2D([], [], marker="o", linestyle="None", markerfacecolor="#9CA3AF", markeredgecolor="#111111", markersize=math.sqrt(size), label=f"{agents} agents")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Plot CUBE COOP2 performance-cost tradeoff.")
    parser.add_argument("--input", type=Path, required=True)
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("experiment/results/figures/cube_performance_cost_tradeoff.pdf"),
    )
