"""Build the CUBE COOP2 aggregate results table."""
...
def _process_constraint_violation_rates(folder: Path) -> Dict[str, float]:
    process_log = read_json(folder / "coop2_process_log.json", [])
...
def load_rows(results_dir: Path, dedupe_latest: bool = True) -> pd.DataFrame:
    ...
    for folder in results_dir.iterdir():
        ...
        process_path = folder / "coop2_process_log.json"
        if not (metrics_path.exists() and score_path.exists() and usage_path.exists() and process_path.exists()):
            continue

        metrics = read_json(metrics_path, {})
        team_score = read_json(score_path, {})
        process_log = read_json(process_path, [])
        agent_states = read_json(folder / "agent_states.json", {})
        ...
        steps = safe_int(team_score.get("current_step"), process_log_max_step(process_log))
        ...
        row = {
            ...
            **metadata,
            "score": score_from_team_score(team_score),
            "steps": steps,
            "total_plans": _plan_count(folder / "plan_logs.json"),
            "messages": safe_int(communication.get("L_tot")),
            "interruptions": _agent_interrupt_count(agent_states),
            "total_decision_time": sum(safe_float(value) for value in decision_per_agent.values()),
    ...
    model_order = DEFAULT_MODELS + sorted(set(df["model"]) - set(DEFAULT_MODELS))
...
def _fmt_mean_ci(row: pd.Series, metric: str, decimals: int = 1) -> str:
    mean = safe_float(row.get(metric))
    ci = safe_float(row.get(f"{metric}_ci95"))
    if ci <= 0:
...
def dataframe_to_markdown(df: pd.DataFrame) -> str:
    ...
    rows = [[str(value).replace("\n", " ") for value in row] for row in df.to_numpy().tolist()]
...
def dataframe_to_latex(
    df: pd.DataFrame,
    *,
    caption: str = "Single-run CUBE results.",
    label: str = "tab:cube_single_run",
) -> str:
    if df.empty:
        return ""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the CUBE COOP2 aggregate results table.")
    parser.add_argument(
        "--results-dir",
        type=Path,
        nargs="+",
    ...
    parser.add_argument("--latex-caption", default="Single-run CUBE results.")
    parser.add_argument("--latex-label", default="tab:cube_single_run")
    ...
    if rows.empty:
        print("No completed CUBE runs matched the requested filters.")
        return 1

    raw_path = output_dir / f"cube_coop2_rows_repair_{args.repair}_seed_{seed_label}.csv"
    rows.to_csv(raw_path, index=False)

    aggregate = aggregate_runs(rows, aggregate_repeats=args.aggregate_repeats)
    aggregate_path = output_dir / f"cube_coop2_table_repair_{args.repair}_seed_{seed_label}.csv"
    aggregate.to_csv(aggregate_path, index=False)

    table = format_table(aggregate)
    if args.hide_runs and "Runs" in table.columns:
        table = table.drop(columns=["Runs"])
    markdown_path = output_dir / f"cube_coop2_table_repair_{args.repair}_seed_{seed_label}.md"
    markdown_path.write_text(dataframe_to_markdown(table), encoding="utf-8")
    latex_path = output_dir / f"cube_coop2_table_repair_{args.repair}_seed_{seed_label}.tex"
