...
REVISED_TASK = TaskSpecification(task=Task.COLLECT_STONE, object_type="stone", object_id=99)


def _plan_response(object_id: int, direction: str = "left") -> LLMPlanResponse:
    return LLMPlanResponse(
        task=TaskSpecification(task=Task.COLLECT_WOOD, object_type="tree", object_id=object_id),
...
class ScriptedLLMClient:
    ...
    @staticmethod
    def _usage():
        return {"prompt_tokens": 1, "completion_tokens": 1, "total_tokens": 2, "latency_seconds": 0.0}
    ...
    def generate(self, messages, response_format=None, temperature=0.7):
        ...
        if response_format is LLMPlanResponse:
            return _plan_response(object_id=len(self.calls)), self._usage()
        if response_format is LLMInterruptResponse:
            ...
            if self.decision == InterruptDecision.REPLAN and self.inline_plan:
                new_plan = LLMPlanResponse(
                    task=REVISED_TASK,
                    actions=[MoveAction(direction="right", num_steps=1)],
                    reasoning="scripted",
                )
...
def _committed_plan(agent_id: str) -> SymbolicPlan:
    return SymbolicPlan(
        specification="collect_wood(tree#1)",
...
class CentralizedFollowerInterruptTests(unittest.TestCase):
    def _interrupt_follower(self, decision, inline_plan=True):
        ...
        # Leader finished its plan and is reasoning again: it broadcasts and waits for responses.
        leader.set_unready(reason="plan_terminated")
        leader.send_message(
            recipients=leader.send_to,
            content="leader planning request",
    ...
    def test_resume_keeps_committed_plan_and_replies_to_leader(self):
...
class BroadcastChainInterruptTests(unittest.TestCase):
    def _interrupt_later_speakers(self, decision):
        ...
        first.send_message(
            ...
            metadata={"type": "broadcast_chain", "speaker_order": 0, "interrupts_execution": True},
        )
    ...
    def test_replan_broadcasts_revised_plan_to_following_speakers(self):
        ...
        self.assertIn(str(REVISED_TASK), third.message_buffer[-1]["content"])
...
class IndividualAgentTests(unittest.TestCase):
    ...
    def test_reasoning_default_plans_with_buffered_messages(self):
        ...
        self.assertIn("hello from agent_0", client.last_prompt[1]["content"])
...
class InterruptPromptTests(unittest.TestCase):
    def test_memory_and_extra_context_are_rendered(self):
        memory = [
            ...
            {"type": "plan", "env_step": 4, "plan_id": 1, "specification": "collect_wood(tree#1)", "actions": []},
        ]
        ...
        self.assertIn("Plan #1: collect_wood(tree#1)", user_prompt)
