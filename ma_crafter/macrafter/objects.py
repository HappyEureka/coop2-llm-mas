...
class Object:
  ...
  def distance(self, target):
    if hasattr(target, 'pos'):
      target = target.pos
    return np.abs(target - self.pos).sum() # manhattan distance
...
class Player(Object):
  ...
  def __init__(self, world, pos, mode, bag_capacity=50, collectable_weights={}):
    ...
    self._fatigue = 0
    self._recover = 0
    self.mode = mode['agent_mode']
    self.bag_capacity_total = bag_capacity
    
    self.bag_capacity_available = bag_capacity
