...
class Env(ParallelEnv):
    def __init__(
        ...
        env_mode={
            "mode": "default",          # [default, grassland]
            "agent_mode": 'default',    # [default, invincible]
            "night": True,              # [True, False]
            "tech_tree": "default",     # [default, no_crafting, no_crafting_no_smelting]
            "seed": None}               # opensimplex seed for world generation
    ):
        ...
        self._action_space = Discrete(len(constants.actions)) #self._action_space = Discrete(1+4+1+1+1) # only allow noop, moves, do, place_table, make_wood_pickaxe
        self.player_id = 0 # for gui only, view of the player
    ...
    def reset(self, seed=42, options=None):
        ...
        worldgen.generate_world(self._world, mode=self.env_mode['mode'])

        self._episode += 1
        self._step = 0
        self._update_time()
        self._unlocked = set()
        self.agents = self.possible_agents[:]  # tracks alive players

        coordinates = np.column_stack(np.where(self._sem_view() == constants.GRASS_CONST))
        locations_indices = np.random.choice(coordinates.shape[0], self.n_players, replace=False)
        spawn_locations = [tuple(coordinates[i]) for i in locations_indices]

        for i, spawn_location in enumerate(spawn_locations):
            self._players[i] = objects.Player(self._world, spawn_location, mode=self.env_mode)
    ...
    def render_one_player(self, player_id, LocalViews, size=None):
        size = size or self._size
        unit = size // self._view
        canvas = np.zeros(tuple(size) + (3,), np.uint8)
        curr_player = self._players[player_id]

        local_view = LocalViews.slice_player_view(curr_player)
        item_view = self._item_view(curr_player.inventory, unit)

        #("local_view shape: ", local_view.shape)
        #print("item_view shape: ", item_view.shape)
    ...
    def render_all(self, size=None):
        self.canvases = []
        unit = (size or self._size) // self._view
        LocalViews = self._local_view(self._players, unit)

        for player_id in range(len(self._players)):
            self.canvases.append(self.render_one_player(player_id, LocalViews, size))
